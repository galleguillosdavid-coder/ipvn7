...
// CaptureFrame captures a single screenshot and encodes it as compressed JPEG bytes
func (rds *RemoteDesktopService) CaptureFrame() ([]byte, error) {
	if runtime.GOOS != "windows" {
		// Fallback test frame for non-windows
		img := image.NewRGBA(image.Rect(0, 0, 640, 480))
		var buf bytes.Buffer
		_ = jpeg.Encode(&buf, img, &jpeg.Options{Quality: rds.quality})
...
func (rds *RemoteDesktopService) InjectInput(evt *RemoteInputEvent) error {
	if !rds.allowControl || runtime.GOOS != "windows" {
		return nil
	}
